...
В этого бота можно кидать тексты больше 4кт символов. Телеграмм их режет на части а бот склеивает обратно и отвечает на них как на цельные сообщения большого размера.
...
Может понимать (переводить голос в текст) аудио сообщения любого размера.
...
Чат-бот автоматически переключается на резервную модель если основная не ответила, и ответ от резервной помещается в память основной как будто это она ответила, это делает бота более живучим и помогает преодолеть проблемы связанные с цензурой.

При пересказе текста по ссылке читает не только текст по ссылке но и все ссылки какие есть в этом тексте. Это помогает более качественно отвечать на вопросы по тексту.
