...
Если в тексте только маты и неприемлимый для тебя контент то просто запикай всё звездочками. \
...
#You can search for this status code onl1ne if you'd like: ALL SYSTEMS. GO"""))
    ...
    print(clear_after_stt('Ну и что это значит блять.'))
    print(clear_after_stt('Ну и что это значит блять. Да ебаный ж ты нахуй.'))
    print(clear_after_stt('пошёл нахуй блять'))
    print(clear_after_stt("Доброго дня! Я готовий допомогти вам з будь-якими питаннями, пов'язаними з моїм функціоналом."))