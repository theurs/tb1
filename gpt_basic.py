...
def translate_text(text, fr = None, to = 'ru'):
    ...
    if fr:
        prompt = f'Исправь явные опечатки в тексте, и переведи с языка ({fr}) на язык ({to}). Покажи только перевод без оформления и отладочной информации. Текст: '
    else:
        prompt = f'Исправь явные опечатки в тексте, и переведи на язык ({to}). Покажи только перевод без оформления и отладочной информации. Текст: '
    prompt += text
