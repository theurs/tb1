...
class BingArt:
    ...
    def _fetch_images(self, encoded_query, ID, IG):
        images = []
        while True:
            response = self.session.get(f'{self.base_url}/async/results/{ID}?{encoded_query}&IG={IG}&IID=images.as'.replace('&amp;nfy=1', ''), timeout=self.timeout)
    ...
    def generate_images(self, query):
        ...
        response = self.session.post(creation_url, data={'q': query}, timeout=self.timeout)
        try:
