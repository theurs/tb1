...
def update_user_bio(new_info: str, user_id: str) -> None:
    '''Update user bio.
    '''
    pass
...
def chat(query: str,
    ...
    try:
        ...
        model_ = genai.GenerativeModel(model,
                                    tools=[search_google, download_text_from_url],
...
if __name__ == '__main__':
    ...
    print(download_texts_from_urls(['https://stackoverflow.com/questions/32908639/open-pil-image-from-byte-file','https://www.youtube.com/watch?v=Zrx1O_OXHdg'], language='ru'))

    # print(detect_lang('Я люблю программировать'))

    # print(sum_big_text(open('1.txt', 'r', encoding='utf-8').read(), 'перескажи в 10 предложениях'))

    # print(reprompt_image('соседка красивая'))
    # print(reprompt_image('соседка красивая', censored=False, pervert = True))

    # img = open('1.jpg', 'rb').read()
    # print(img2txt(img))

    # как юзать прокси
    # как отправить в чат картинку, и аудиофайл
    # как получить из чата картинки, и аудиофайлы

    chat_cli()

    # print(chat(open('1.txt', 'r', encoding='utf-8').read(), system='сделай краткий пересказ, 500 слов'))

    # print(ai('напиши 3 слова, задом наперед', mem = MEM_UNCENSORED))
    # print(ai('напиши 3 слова'))
