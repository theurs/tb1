...
def gen_images(prompt: str, moderation_flag: bool = False, user_id: str = '', conversation_history: str = ''):
    ...
    async_result7 = pool.apply_async(stability_ai, (prompt,))
    async_result8 = pool.apply_async(stability_ai, (prompt,))

    result = (async_result1.get() or []) + (async_result5.get() or []) + (async_result6.get() or []) + \
        (async_result7.get() or []) + (async_result8.get() or []) + \
        (async_result2.get() or []) + (async_result3.get() or [])
