...
PROMPT_SOLVE = 'Solve all problems presented in the image. Show your step-by-step solution and clearly indicate the final answer. Use Unicode characters for mathematical notation to ensure accurate representation, never use symbols like <0xE1><0x83><0xAE><0xE0><0xBF><0xA0> and markdown and latex in the math expressions.'
...
def get_hidden_prompt_for_user(message, chat_id_full, bot_name, lang_of_user, formatted_date):
    hidden_text = (
                    ...
                    f'/trans - translate, /sum - summarize, /google - search, /clone_voice, you can answer voice messages, '
                    ...
                    f'no emoji unless it is required. Use Unicode characters for mathematical notation to ensure accurate representation, never use symbols like <0xE1><0x83><0xAE><0xE0><0xBF><0xA0> and markdown and latex in the math expressions. '
...
def get_hidden_prompt_for_llama(tr, lang):
    return tr(f'Answer in "{lang}" language, do not address the user by name and no emoji unless it is required. Use Unicode characters for mathematical notation to ensure accurate representation, never use symbols like <0xE1><0x83><0xAE><0xE0><0xBF><0xA0> and markdown and latex in the math expressions.', lang)


def get_img2txt_prompt(tr, lang):
    return tr(f'Answer in "{lang}" language, if not asked other. Use Unicode characters for mathematical notation to ensure accurate representation, never use symbols like <0xE1><0x83><0xAE><0xE0><0xBF><0xA0> and markdown and latex in the math expressions.', lang)
