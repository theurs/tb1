...
def get_hidden_prompt_for_user(message, chat_id_full, bot_name, lang_of_user, formatted_date):
    hidden_text = (
                    ...
                    f'you need a user`s api keys or telegram stars for working (/keys for more info), '
                    ...
                    f'you can generate images with the /img command, type /img "prompt" to generate an image for user, '
                    f'you can google search with the /google command, type /google "question" to get the answer for user, '
                    # f'use a special function to perform complex calculations, the task must be formulated in ordinary human language, no matter how difficult it is to solve, just say yourself(dont`t ask user to say) /calc "the task as a person would formulate it" to get the answer, '
