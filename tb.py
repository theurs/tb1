...
def img2txt(text, lang: str, chat_id_full: str, query: str = '') -> str:
    ...
    if not query:
        query = tr('Что изображено на картинке? Если основная часть это текст то просто извлеки и покажи этот текст, сохраняя по возможности его форматирование. Если основная часть это не текст то напиши подробное описание, и объясни подробно что это может означать, затем напиши длинный подробный промпт одним предложением для рисования этой картинки с помощью нейросетей, начни промпт со слов /image Create image of...', lang)
    else:
