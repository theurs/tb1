...
'''
    ...
    if not query:
        use_json = True
        query = tr('Что изображено на картинке? Если там написан вопрос или задание то дай на него ответ. Дай подробное описание и профессиональный промпт для генерации этого изображения на английском языке.', lang) + '\n\n' + tr(f'Answer in "{lang}" language, if not asked other.', lang) + json_query
    else:
        use_json = False
