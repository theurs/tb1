...
# пока нигде не используется но может быть полезна для разделения текста на мелкие части которые способен переварить GPT chat
def split_text(text: str, chunk_limit: int = 1500):
    """разбивает текст на части заданной длинны стараясь не порвать слова на части и сделать разрыв на конце предложения"""
    result = []
    current_chunk = ""
    for sentence in text.split("."):
        if len(current_chunk + sentence) + 1 <= chunk_limit:
    ...
    if len(result) == 1:
        ...
        current_chunk = ""
        for word in words:
            if len(current_chunk + word) + 1 <= chunk_limit:
